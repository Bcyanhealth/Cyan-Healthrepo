...
const COLS = {
  ...
  patient: { ts:0, id:1, name:2, email:3, dob:4, age:5, gender:6, phone:7, address:8, reason:9, consent:10, folder:11 },
  consult: { ts:0, id:1, email:2, dov:3, cc:4, hpi:5, pmh:6, vitals:7, exam:8, dx1:9, dx2:10, investigations:11, referral:12, clinician:13, signature:14 },
  tele:    { ts:0, name:1, phone:2, service:3, date:4, time:5, notes:6  },
  pay:     { ts:0, id:1, name:2, service:3, amount:4, method:5, ref:6, by:7 },
  revenue: { ts:0, name:1, id:2, date:3, service:4, amount:5, method:6, receipt:7 } ,
  follow:  { ts:0, name:1, phone:2, service:3, date:4, time:5, notes:6 }
};

function onOpen() {
  SpreadsheetApp.getUi().createMenu("HMIS")
    .addItem("🔧 Check Setup", "checkSetup")
    .addItem("📁 Backfill Patient Folders", "backfillPatientFolders")
    .addSeparator()
    .addItem("📅 Rebuild Follow-up Reminder Trigger", "installDailyFollowupTrigger")
    .addToUi();
}
...
function checkSetup() {
  const s = getSettings();
  const missing = [];
  ["ROOT_FOLDER_ID","CALENDAR_ID"].forEach(k => { if (!s[k]) missing.push(k); });
...
function onConsultationSubmit(e) {
  ...
  if (folderUrl) {
    ...
    if (folderId) {
      ...
      const doc = DocumentApp.create(`Consultation_${patientID}_${new Date().toISOString().slice(0,10)}`);
  ...
  upsertFollowup(patientID, fullName, "", "", new Date(), addDays(new Date(), 30), "Pending");
}

function upsertFollowup(id, name, email, phone, lastVisitDate, nextDate, status) {
  ...
  for (let r=1;r<vals.length;r++) {
    if (String(vals[r][c.id]) === id) { foundRow = r+1; break; }
  }
  const rowData = [
    id, name, email||"", phone||"",
...
function onTelemedicineSubmit(e) {
  ...
  const created = Calendar.Events.insert(event, calendarId, { conferenceDataVersion: 1, sendUpdates: "all" });
  const eventId = created.id;
  const meetLink = created.hangoutLink || (created.conferenceData?.entryPoints?.find(e => e.entryPointType==="video")?.uri) || "";
...
function installDailyFollowupTrigger() {
  ...
  ScriptApp.getProjectTriggers().forEach(t => {
    if (t.getHandlerFunction() === "dailyFollowupSweep") ScriptApp.deleteTrigger(t);
  });
  // New daily at 8am
  ScriptApp.newTrigger("dailyFollowupSweep").timeBased().atHour(8).everyDays(1).create();
}
...
function sendWhatsAppReminder(phone, name, date) {
  ...
  try { UrlFetchApp.fetch(url, params); } catch (e) { Logger.log(e); }
}
